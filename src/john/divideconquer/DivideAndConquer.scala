...
object DivideAndConquer {
  ...
  def divide(n: Network, p: Divide): (Set[Constraint], List[NetworkMap]) = {
    ...
    return (crossConstraints, List[NetworkMap](networkMapR, networkMapL))
  }
  ...
  def join(C: Set[Constraint], phi: List[SolutionSet]): SolutionSet = {
    val rss = phi.head.solutions
    val lss = phi.tail.head.solutions
    
    var combinedSolutions = Array[Solution]()
      ...
      for (solutionsr <- rss) {
        ...
        if (solutionL(solutionL.size-1) == solutionR.head) {
          ...
          if (ConstraintUtil.satisfiesConstraints(C, combined)) {
            combinedSolutions = combinedSolutions :+ combined
          }
        }
      }  
    }
    return SolutionSet(combinedSolutions)
  }
}